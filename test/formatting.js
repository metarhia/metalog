...
test('Formatter with JSON mode', () => {
  ...
  if (parsed.level !== 'error') {
    throw new Error('JSON level incorrect');
  }
  if (parsed.message !== 'Test JSON message') {
    throw new Error('JSON message incorrect');
  }
  if (parsed.workerId !== 'W2') {
    throw new Error('JSON workerId incorrect');
  }
});
...
test('Formatter with different log types', () => {
  const formatter = new Formatter({
    json: false,
    workerId: 'W5',
    home: '/test/home',
  ...
  for (const type of types) {
    const formatted = formatter.format(type, 0, ['Test message']);
    if (!formatted.includes('Test message')) {
      throw new Error(`Format failed for type: ${type}`);
    }
  }
});
